...
def augmentations(x_resize, y_resize):
    return A.Compose([
        ...
        A.RandomBrightnessContrast(brightness_limit=0.1, contrast_limit=0.1, brightness_by_max=True, p=0.2)
...
def main():
    ...
    # train on field data:
    dataset_train = FliesDataset(flies_dir, csv_train, get_transform(), augmentations(x_resize, y_resize))
    dataset_val = FliesDataset(flies_dir, csv_val, get_transform(), test_augmentations(x_resize, y_resize))
    dataset_test = FliesDataset(flies_dir, csv_test, get_transform(), test_augmentations(x_resize, y_resize))
