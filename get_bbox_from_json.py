    ...
    for root, dirs, files in os.walk(dr):
        for index, file in enumerate(files):
            if os.path.splitext(file)[1] == '.json':
                ...
                assert image_name not in images, f"image {image_name} already exists. file name: {file}"
                images.append(image_name)
                assert len(json_file) == 1, f"Encountered more than one image in JSON file: {path}"
                ...
                for bbox in json_data[:-1]:
                    ...
                    if 0 < label < 12:
                        assert area > 0, f"Wrong bounding box size: {box_data}, file name: {path}"
