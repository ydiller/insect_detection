...
def augmentations(x_resize, y_resize):
    return A.Compose([A.Resize(x_resize, y_resize, interpolation=cv.INTER_AREA)])
...
def main():
    ...
    dataset_test = TestDataset(
        dr, img_paths, get_transform(), augmentations(x_resize, y_resize)
    )
    ...
    checkpoint = torch.load(
        opt.model_load_path, map_location=lambda storage, loc: storage
    )
